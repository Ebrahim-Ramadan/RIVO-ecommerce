import { getFirestore, collection, getDocs, getDoc, doc ,where, query, limit} from 'firebase/firestore';
  ...
  export async function getProductDetails(docId) {
    try {
      ...
      if (docSnap.exists()) {
        ...
        if (data.types) data.types = data.types.split(',').map(item => item.trim());
