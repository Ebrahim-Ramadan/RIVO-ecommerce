...
export const copyToClipboard = (text) => {
  ...
  export const getAllFrames = async () => {
    try {
      ...
      const framesQuery = query(framesCollection, limit(50));
  ...
  export const getFramesByCategory = async (categoryParam) => {
      ...
      // const cacheKey = `frames_${lowercaseCategoryParam}`;
  
      // if (typeof window !== 'undefined') {
      //     // Check if the data is already in localStorage (client-side only)
      ...
      try {
          ...
          querySnapshot.forEach((doc) => {
              ...
              if (data.color) data.color = data.color.split(',').map(item => item.trim());
              if (data.categories) {
                  data.categories = data.categories.split(',').map(item => item.trim());
                  if (data.categories.map(c => c.toLowerCase()).includes(lowercaseCategoryParam)) {
                      if (data.sizes) {
          ...
          // if (typeof window !== 'undefined') {
          //     // Cache the results in localStorage (client-side only)
          //     localStorage.setItem(cacheKey, JSON.stringify(filteredFrames));
          // }
