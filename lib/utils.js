...
export const copyToClipboard = (text) => {
  ...
  export async function getProductDetails(docId) {
    try {
      ...
      if (docSnap.exists()) {
        ...
        data.keywords = data.keywords.split(',').map(item => item.trim().toLowerCase());
      }
  ...
  export const searchFrames = async (keyword, related, relatedID) => {
    try {
      ...
      // Generate substrings for the search keyword with a minimum length
      ...
      querySnapshot.forEach((doc) => {
        ...
        if (data.keywords) {
          ...
          // Check if the search keyword or meaningful substrings are included in the keywords
