...
export const searchFrames = async (keyword) => {
  ...
  export const updateDocumentsWithVinyls = async () => {
    try {
      ...
      // Query documents where the 'categories' array contains 'cairokee frames'
      const q = query(collectionRef, where('categories', 'array-contains', 'cairokee frames'));
      const querySnapshot = await getDocs(q);
  
      for (const docSnapshot of querySnapshot.docs) {
        const documentData = docSnapshot.data();
  
        // Check if the document has a 'keywords' field
