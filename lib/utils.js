...
export const searchFrames = async (keyword) => {
  ...
  export const updateDocumentsWithVinyls = async () => {
    try {
      ...
      const q = query(collectionRef, where('categories', 'array-contains', 'posters set'));
      const querySnapshot = await getDocs(q);
...
export const deleteDocs = async () => {
  try {
    ...
    // Query documents where the 'categories' array contains 'posters set'
    const q = query(collectionRef, where('categories', 'array-contains', 'posters set'));
    const querySnapshot = await getDocs(q);
    ...
    for (const docSnapshot of querySnapshot.docs) {
      ...
      console.log(`Document ${docSnapshot.id} deleted successfully.`);
    }
  } catch (error) {
